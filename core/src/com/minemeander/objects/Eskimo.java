...
public class Eskimo extends GameObject implements Climber {

	private final static float ESKIMO_SPEED = 6.0f;
	private final static float ESKIMO_PURSUIT_DISTANCE = 20.0f;
	private final static float ESKIMO_MAX_TIME_TO_REACH_WAYPOINT = 30.0f; // sec
	...
	final static Vector2 upImpulseVector = new Vector2(0.0f, ESKIMO_SPEED * 3);
	final static Vector2 jumpRightImpulseVector = new Vector2(ESKIMO_SPEED * 50, ESKIMO_SPEED * 50);
	final static Vector2 jumpLeftImpulseVector = new Vector2(-ESKIMO_SPEED * 50, ESKIMO_SPEED * 60);
	final static Vector2 downImpulseVector = new Vector2(0.0f, -ESKIMO_SPEED * 3);
	final static Vector2 leftImpulseVector = new Vector2(-ESKIMO_SPEED, 0.0f);
	final static Vector2 rightImpulseVector = new Vector2(ESKIMO_SPEED, 0.0f);

	public Eskimo(int id, Level level, float x, float y) {
		super(id, level, x, y, CollisionCategory.ENEMY, true);
	...
	public void renderDebugInfo() {
		...
		if (wayPointList != null) {
			...
			for (int i = wayPointList.size() - 1; i >= 0; i--) {
				...
				float x1 = wayPointNode.x * Constant.METERS_PER_TILE;
				float y1 = wayPointNode.y * Constant.METERS_PER_TILE;
	...
	public TextureRegion getTextureRegion(float tick) {
		...
		if (wayPointList.size() == 0 || (tick - targetTileSelectionTimestamp) > ESKIMO_MAX_TIME_TO_REACH_WAYPOINT) {
	...
	private Vector2 distanceToWayPoint(PathNode pathNode) {
		Vector2 position = body.getPosition();
		float tileX = pathNode.x * Constant.METERS_PER_TILE + 1;
		float tileY = pathNode.y * Constant.METERS_PER_TILE + 1;
	...
	public static enum ZombieAnimationEnum {
		IDLE(Art.eskimoWalkingLeftAnimation), WALK_LEFT(Art.eskimoWalkingLeftAnimation), WALK_RIGHT(Art.eskimoWalkingRightAnimation), CLIMBING(Art.eskimoClimbAnimation);
